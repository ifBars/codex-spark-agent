...
pub(crate) use policy::ToolAccessPolicy;
...
pub(crate) fn invoke_local_read_with_read_roots(
    cwd: &Path,
    read_roots: &[PathBuf],
    mode: AgentMode,
    tool_name: &str,
    ...
    let result = (|| -> Result<ToolResult> {
        ...
        match tool_name {
            ...
            _ => anyhow::bail!("tool `{tool_name}` is not a parallel-safe local read"),
        }
    })();
    result.unwrap_or_else(|error| ToolResult {
        ok: false,
        data: structured_tool_error(tool_name, &error_args, &error.to_string()),
        error: Some(error.to_string()),
    })
}
...
async fn invoke_inner(
    ...
    match tool_name {
        ...
        "gh.read" => gh_read(cwd, args).await,

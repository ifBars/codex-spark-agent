...
export function reduceWave1MeasurementState(state, action) {
  switch (action.type) {
    ...
    case "session": return { ...state, phase: "active", capture: action.capture, fixture: action.fixture ?? state.fixture, retention: action.retention, sessionNamespace: action.sessionNamespace, purged: false, error: null };
    case "ack": return { ...state, acknowledgements: { ...state.acknowledgements, [action.eventType]: action.acknowledgement }, error: null };
    case "aggregate": return { ...state, aggregate: action.aggregate, retention: action.aggregate.retention, error: null };
    case "purged": return { ...state, phase: "preflight", retention: action.retention, sessionNamespace: action.nextSessionNamespace, acknowledgements: {}, aggregate: null, purged: true, error: null };
